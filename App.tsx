...
import {
  ...
  DIOMEDES_IMAGE_URL,
  LEGO_IMAGE_URL,
  PATAGONIA_VOUCHER_IMAGE_URL,
  PERFUME_IMAGE_URL,
  WATCH_IMAGE_URL,
  PIJAMA_IMAGE_URL, // Import Pijama image
  VASO_IMAGE_URL,   // Import Vaso image
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const imagesToPreload = [
        ...
        PERFUME_IMAGE_URL,
        WATCH_IMAGE_URL,
        PIJAMA_IMAGE_URL, // Add Pijama image to preload
        VASO_IMAGE_URL,   // Add Vaso image to preload
  ...
  const handleAnswerSubmit = () => {
    ...
    if (userAnswer.toLowerCase().trim() === currentRiddle.answer.toLowerCase().trim()) {
      setFeedbackMessage(`¡Ajá! ¡Esa es! ¡Acertaste!`); 
      ...
      <div className="animate-fadeIn">
        {currentRiddle.from && (
          <p className="text-3xl font-semibold text-white mb-3">
            De parte de: {currentRiddle.from}
          </p>
        )}
        ...
        {gameState === GameState.RiddleCorrect && (
          <div className="mt-4 p-4 bg-slate-700/70 rounded-lg shadow-md animate-fadeIn">
            <h3 className={`text-xl font-bold ${currentRiddle.isLego ? 'text-colombia-yellow' : 'text-colombia-blue-light'}`}>{currentRiddle.giftName}</h3>
            <p className="text-slate-300 mt-1">{currentRiddle.giftDescription}</p>
            {showSpecialImage && (
              <img
                src={showSpecialImage}
                alt={getDynamicAltText(showSpecialImage, gameState, currentRiddle)}
                className="mt-4 rounded-lg shadow-xl max-w-xs mx-auto max-h-64 h-auto w-auto object-contain" />
            )}
          </div>
        )}

        {gameState === GameState.RiddleIncorrect && showSpecialImage && (
           <img
            src={showSpecialImage}
            alt={getDynamicAltText(showSpecialImage, gameState, currentRiddle)}
        ...
        {revealedGifts.length > 0 ? (
          revealedGifts.map(gift => (
            <div key={gift.id} className="bg-slate-800 p-4 rounded-lg shadow-lg transition-all duration-300 hover:shadow-xl transform hover:scale-102">
              ...
              {gift.from && (
                <p className="text-sm text-sky-400 italic mt-1">Un detallito de: {gift.from}</p>
              )}
