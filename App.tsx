...
const App: React.FC = () => {
  ...
  const handleAnswerSubmit = () => {
    ...
    if (userAnswer.toLowerCase().trim() === currentRiddle.answer.toLowerCase().trim()) {
      ...
      // Add gift only if it hasn't been added for this riddle index before
      // This check might be redundant if button disabling works correctly, but good for safety
      if (!revealedGifts.find(gift => gift.id === currentRiddle.id && revealedGifts.filter(g => g.id === currentRiddle.id).length > (currentRiddleIndex === RIDDLES.findIndex(r => r.id === currentRiddle.id) ? 0 : -1))) { // This logic for checking duplicates is a bit complex. The button disabling should prevent this.
  ...
  const handleNextAction = () => {
    ...
    if (gameState === GameState.RiddleCorrect) {
      ...
      setGameState(GameState.Playing); // Reset to Playing to allow another attempt for the same riddle.
    }
  };
